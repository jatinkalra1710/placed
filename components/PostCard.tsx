...
export default function PostCard({
  ...
  const date = new Date(post.created_at).toLocaleString("en-IN", {
    day: "numeric",
    month: "short",
    hour: "2-digit",
    minute: "2-digit",
        ...
        <div className="flex items-center gap-2">
          ...
          {!isOwner && currentUserId && (
            <button
              ...
              className="text-[11px] text-slate hover:text-rust"
              title="Report post"
            >
              Report
            </button>
      ...
      <AnimatePresence>
        {showReport && (
          <motion.div
            ...
            className="mt-3 overflow-hidden rounded border border-rust/40 bg-rust/5 p-3"
          >
            {reportSent ? (
              <p className="text-xs text-signal">Thanks — an admin will take a look.</p>
            ) : (
              <>
                <textarea
                  value={reportReason}
                  ...
                  rows={2}
                  className="w-full rounded border border-rust/40 bg-ink px-2 py-1.5 text-xs text-paper outline-none"
                />
                <button
                  onClick={submitReport}
                  disabled={!reportReason.trim()}
                  className="mt-2 rounded-full bg-rust px-3 py-1 text-[11px] font-semibold text-ink hover:bg-rust/90 disabled:opacity-50"
                >
                  Submit report
                </button>
              </>
                ...
                <form onSubmit={sendReply} className="mt-2 space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="rounded-full bg-amber px-3 py-1.5 text-xs font-semibold text-ink hover:bg-amber/90 disabled:opacity-50"
                    >
