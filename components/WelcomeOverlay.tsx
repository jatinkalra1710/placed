    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-ink/80 backdrop-blur-sm"
        ...
        >
          {dots.map((_, i) => (
            <motion.span
              ...
              style={{
                background: i % 2 === 0 ? (cityColor || "rgb(var(--color-amber))") : "rgb(var(--color-signal))",
              }}
            />
          ))}
          ...
          >
            ...
            <button
              onClick={() => setShow(false)}
              className="mt-6 w-full rounded-full bg-amber px-5 py-2.5 text-sm font-semibold text-ink hover:bg-amber/90"
            >
              Let's go
            </button>
          </motion.div>
