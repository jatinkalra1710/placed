-- ============================================================
-- Thapar Placed — automated welcome email on approval.
-- Sends via Brevo's transactional email API the moment an admin
-- flips a verification's status to 'approved'. Uses pg_net, so no
-- Edge Function or external server is needed — it fires straight
-- from Postgres.
-- ============================================================

-- 1) Turn on pg_net (one-time, safe to re-run)
create extension if not exists pg_net with schema extensions;

-- 2) Store your Brevo API key securely in Supabase Vault (NOT as a
--    plain column — vault encrypts it at rest). Get the key from
--    Brevo → SMTP & API → API Keys → Create a new API key.
--    Run this once, replacing the placeholder:
select vault.create_secret('YOUR_BREVO_API_KEY_HERE', 'brevo_api_key');

-- If you ever need to update it later instead of creating fresh:
-- select vault.update_secret(
--   (select id from vault.secrets where name = 'brevo_api_key'),
--   'YOUR_NEW_KEY'
-- );

-- 3) The function that actually sends the email
...
declare
  ...
  v_html text;
begin
  -- only fire on the transition INTO approved, not every update
...
-- 4) Wire it up to fire automatically on approval
