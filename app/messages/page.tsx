  ...
  useEffect(() => {
    async function load() {
      ...
      for (const m of data ?? []) {
        ...
        const incomingUnread = m.receiver_id === myId && !m.read_at;
        if (!byOther.has(otherId)) {
          ...
          // any unread message in the thread marks the whole conversation unread
          byOther.get(otherId)!.unread = true;
        }
      }
        ...
        <div className="mt-6 space-y-2">
          {conversations.map((c, i) => (
            <motion.a
              ...
              className="flex items-center gap-3 glass-card p-4 transition-colors hover:border-amber/40"
            >
              <span
                className={`h-2 w-2 shrink-0 rounded-full ${c.unread ? "bg-rust" : "bg-transparent"}`}
              />
              <div className="min-w-0 flex-1">
                <p className={`truncate ${c.unread ? "font-semibold text-paper" : "font-medium text-paper/90"}`}>
                  {c.otherName}
                </p>
                <p className={`mt-0.5 truncate text-sm ${c.unread ? "text-paper/80" : "text-slate"}`}>
                  {c.lastMessage}
                </p>
              </div>
            </motion.a>
